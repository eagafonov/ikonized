...
// Modified by: Ron Lee, Eugene Agafonov
// Created:
...
// FIXME #include "wx/wxprec.h"

// FIXME #include "wx/display.h"
#include "sizer.h"
...
#define wxCHECK_RET(A, B) \
                        if (A == 0) \
                        {           \
                            return; \
                        }       
#define wxFAIL_MSG(A)
#define wxCHECK_MSG(A, B, C)
#define wxIsNullDouble(A) (A == 0)
#define wxASSERT_MSG(A, B)
    ...
    switch ( m_kind )
    {
        ...
        case Item_Sizer:
            ...
            // deprecating this function)
            {
                ...
                for ( wxSizerItemList::iterator node = m_sizer->GetChildren().begin();
                      node != m_sizer->GetChildren().end();
...
wxSizerItem* wxSizer::Append(wxSizerItem *item )
{
...
void wxSizer::SetContainingWindow(wxWindow *win)
{
    ...
    for ( wxSizerItemList::iterator node = m_children.begin();
          ...
          node++ /*node = node->GetNext()*/ )
    {
        wxSizerItem *const item = *node; /*node->GetData()*/;
...
bool wxSizer::Remove( wxSizer *sizer )
{
    ...
    wxSizerItemList::iterator node = m_children.begin();
    while (node != m_children.end())
    {
        wxSizerItem     *item = *node; /*node->GetData();*/
...
bool wxSizer::Remove( int index )
{
    ...
    //wxSizerItemList::iterator node = m_children.Item( index );
...
//
//    wxSizerItemList::iterator node = m_children.Item( old );
//
//    wxCHECK_MSG( node, false, _T("Failed to find child node") );
//
//    wxSizerItem *item = *node; //node->GetData();
//    *node = newitem; // node->SetData(newitem);
//    
//    delete item;
//
//    return true;
...
wxSizerItem* wxSizer::GetItem( size_t index )
{
    ...
    //return *(m_children.Item(index)); // m_children.Item( index )->GetData();
}
...
{
    wxSizerItemList::const_iterator node = m_children.begin();
...
//
//    return (*(m_children.Item( index )))->IsShown(); // m_children.Item( index )->GetData()->IsShown();
//}
...
{
    int nitems = (int)m_children.size();
...
void wxGridSizer::RecalcSizes()
{
    ...
    for (int c = 0; (c < ncols) && (node != m_children.end()); c++)
    {
        int y = pt.y;
        for (int r = 0; (r < nrows) && (node != m_children.end()); r++, node ++)
        {
            int i = r * ncols + c;
            ...
            if (i < nitems)
            {
                ...
                assert(node != m_children.end());

                SetItemBounds( *node /*node->GetData()*/, x, y, w, h);
            }
            y = y + h + m_vgap;
        }
        x = x + w + m_hgap;
...
void wxFlexGridSizer::RecalcSizes()
{
    ...
    //        if (i < nitems)
    //        {
    //            wxSizerItemList::iterator node = m_children.Item( i );
